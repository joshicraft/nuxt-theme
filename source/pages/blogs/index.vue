    ...
    <div class="columns is-multiline">
      <div v-for="(blog,index) in blogs" :key="blog.id" :data-aos="index ? 'fade-up' : 'fade'" class="column is-full mb4">
        <figure class="image mb3">
          <ul v-if="isArray(blog.tags)" class="tags absolute top-1 right-1">
            <li class="tag is-size-8 is-secondary ttu" v-for="tag in blog.tags" :key="tag.id" >{{ tag.title }}</li>
          </ul>

          <ul v-if="isObject(blog.tags)" class="tags absolute top-1 right-1">
            <li class="tag is-size-8 is-secondary ttu" :key="blog.tags.id">{{ blog.tags.title }}</li>
          </ul>
...
<script>
  ...
  export default {
    ...
    mixins: [Helper],
    created () {
      this.$store.commit('SET_PAGE_TITLE', "Blogs")
    },
    components: {
      Pagination,
    },
